...
class FIIStrengthCalculatorRespective:
    ...
    def calculate_respective_percentiles(self, historical_data):
        ...
        # Combined percentiles for call and put options
        if 'CALL_OI' in combined_data.columns and 'PUT_OI' in combined_data.columns:
            ...
            if len(combined_oi) > 0 and len(combined_change) > 0:
                ...
                print(f"  Num OI points (Call + Put): {len(call_oi)} + {len(put_oi)} = {len(combined_oi)}, Num Change points (Call + Put): {len(call_change)} + {len(put_change)} = {len(combined_change)}")
                print(f"  OI p80: {percentiles['Call Options']['oi_p80']:.0f}, p40: {percentiles['Call Options']['oi_p40']:.0f}")
                print(f"  Change p80: {percentiles['Call Options']['change_p80']:.0f}, p20: {percentiles['Call Options']['change_p20']:.0f}")
    ...
    def calculate_accuracy_summary(self, lookback_days=60):
        ...
        all_dates = sorted(self.df['date'].unique(), reverse=True)
        analysis_dates = all_dates[:lookback_days - 1]  # Exclude last to have next day
