...
class FIIStrengthCalculatorRespective:
    ...
    def calculate_respective_percentiles(self, historical_data):
        """Calculate respective percentiles for call and put options separately"""
        ...
        if 'CALL_OI' in combined_data.columns:
        ...
        if 'PUT_OI' in combined_data.columns:
    ...
    def calculate_strength_for_date(self, target_date, combined_percentiles):
        ...
        for institution in ['FII', 'PRO', 'CLIENT']:
            ...
            if 'CALL_OI' in inst_data.columns and 'Call Options' in combined_percentiles:
                segments.append(('Call Options', 'CALL_OI', 'CALL_OI_CHANGE', 'Call Options'))
            
            if 'PUT_OI' in inst_data.columns and 'Put Options' in combined_percentiles:
    ...
    def calculate_last_n_days_strength(self, n_days=3, lookback_days=60):
        ...
        combined_percentiles = self.calculate_respective_percentiles(historical_data)
        ...
        # Options: Call and Put separate
        if 'CALL_OI' in combined_data_print.columns:
            call_oi = combined_data_print['CALL_OI'].dropna()
            call_change = combined_data_print['CALL_OI_CHANGE'].dropna()
            
            if len(call_oi) > 0 and len(call_change) > 0 and 'Call Options' in combined_percentiles:
                p_call = combined_percentiles['Call Options']
        ...
        if 'PUT_OI' in combined_data_print.columns:
            put_oi = combined_data_print['PUT_OI'].dropna()
            put_change = combined_data_print['PUT_OI_CHANGE'].dropna()
            
            if len(put_oi) > 0 and len(put_change) > 0 and 'Put Options' in combined_percentiles:
                p_put = combined_percentiles['Put Options']
        ...
        # Print the header once for the unified table
        print("Date,Participant,Index Futures OI,Index Futures Change,Index Futures Strength,Call OI,Call OI Change,Call Strength,Put OI,Put Change,Put Strength,Net Options OI,Net Change,Net Strength,Current day NIFTY,NIFTY change %,Next Day NIFTY,Next Day NIFTY Change %")
        ...
        # Prepare NIFTY data lookup
        nifty_dates = self.nifty_df['date'].unique()
        date_to_nifty = dict(zip(self.nifty_df['date'], self.nifty_df['close']))
        date_to_change = dict(zip(self.nifty_df['date'], self.nifty_df['nifty_change_pct']))
        
        # Get sorted trading dates for prev/next lookup
        ...
        for date in reversed(last_n_dates):  # Show oldest to newest
            ...
            # Get NIFTY info for this date
            current_nifty = date_to_nifty.get(date, 'N/A')
            current_change_pct = date_to_change.get(date, 'N/A')
            
            # Find prev and next dates
            date_idx = date_to_idx.get(date)
            if date_idx is not None:
                prev_idx = date_idx - 1
                if prev_idx >= 0:
                    prev_date = sorted_trading_dates[prev_idx]
                    prev_nifty = date_to_nifty.get(prev_date)
                    if prev_nifty is not None:
                        current_change_pct = ((current_nifty - prev_nifty) / prev_nifty * 100) if current_nifty != 'N/A' else 'N/A'
                else:
                ...
                next_nifty = 'N/A'
                next_change_pct = 'N/A'
                if next_idx < len(sorted_trading_dates):
                    next_date = sorted_trading_dates[next_idx]
                    next_nifty = date_to_nifty.get(next_date)
                    if next_nifty is not None and current_nifty != 'N/A':
                        next_change_pct = ((next_nifty - current_nifty) / current_nifty * 100)
            else:
                current_change_pct = 'N/A'
                next_nifty = 'N/A'
                next_change_pct = 'N/A'
            
            # Display results for this date in unified table format (no header repeat)
            for institution in ['FII', 'PRO', 'CLIENT']:
                inst_results = {r['segment']: r for r in results if r['institution'] == institution}
                
                index_data = inst_results.get('Index Futures', {})
                call_data = inst_results.get('Call Options', {})
                put_data = inst_results.get('Put Options', {})
                net_data = inst_results.get('Net Options', {})
                
                # Safely get values, default to 0 or 'N/A'
                def get_val(data, key, default=0):
                    return data.get(key, default) if data else default
                
                def get_str(data, key, default='INDECISIVE'):
                    return data.get(key, default) if data else default
                
                index_oi = get_val(index_data, 'net_oi')
                index_change = get_val(index_data, 'change')
                index_strength = get_str(index_data, 'final_strength')
                
                call_oi = get_val(call_data, 'net_oi')
                call_change = get_val(call_data, 'change')
                call_strength = get_str(call_data, 'final_strength')
                
                put_oi = get_val(put_data, 'net_oi')
                put_change = get_val(put_data, 'change')
                put_strength = get_str(put_data, 'final_strength')
                
                net_oi = get_val(net_data, 'net_oi')
                net_change = get_val(net_data, 'change')
                net_strength = get_str(net_data, 'final_strength')
                
                # Handle N/A for change percentages
                current_change_str = f"{float(current_change_pct):.2f}" if isinstance(current_change_pct, (int, float)) else str(current_change_pct)
                next_change_str = f"{float(next_change_pct):.2f}" if isinstance(next_change_pct, (int, float)) else str(next_change_pct)
                
                # Handle N/A for NIFTY closes (convert to float if possible)
                current_nifty_str = f"{float(current_nifty):.2f}" if isinstance(current_nifty, (int, float)) else str(current_nifty)
                next_nifty_str = f"{float(next_nifty):.2f}" if isinstance(next_nifty, (int, float)) else str(next_nifty)
                
                print(f"{date.date()},{institution},{index_oi},{index_change},{index_strength},{call_oi},{call_change},{call_strength},{put_oi},{put_change},{put_strength},{net_oi},{net_change},{net_strength},{current_nifty_str},{current_change_str}%,{next_nifty_str},{next_change_str}%")
...
def generate_market_summary(results, target_date, api_key):
    """
    Generate comprehensive market summary using OpenRouter Grok 4 fast model
    
    Args:
        results: List of result dictionaries from calculate_strength()
        target_date: Date to generate summary for
        api_key: Your OpenRouter API key
    
    Returns:
        String containing the market summary
    """
    ...
    for result in results:
        if result['date'] == target_date:
            ...
            # Store each segment's data
            segment_key = segment.lower().replace(' ', '_')
            institution_data[institution][segment_key] = {
                'strength': result['final_strength'],
                'net_oi': result['net_oi'],
                'change': result['change']
            }
    
    # Build comprehensive prompt
    prompt = f"""Create a comprehensive market analysis summary for {target_date} based on participant behavior:

FII (Foreign Institutional Investors):
- Index Futures: Strength={institution_data.get('FII', {}).get('index_futures', {}).get('strength', 'N/A')}, Net OI={institution_data.get('FII', {}).get('index_futures', {}).get('net_oi', 0):,.0f}, Change={institution_data.get('FII', {}).get('index_futures', {}).get('change', 0):,.0f}
- Call Options: Strength={institution_data.get('FII', {}).get('call_options', {}).get('strength', 'N/A')}, Net OI={institution_data.get('FII', {}).get('call_options', {}).get('net_oi', 0):,.0f}, Change={institution_data.get('FII', {}).get('call_options', {}).get('change', 0):,.0f}
- Put Options: Strength={institution_data.get('FII', {}).get('put_options', {}).get('strength', 'N/A')}, Net OI={institution_data.get('FII', {}).get('put_options', {}).get('net_oi', 0):,.0f}, Change={institution_data.get('FII', {}).get('put_options', {}).get('change', 0):,.0f}
- Net Options: Strength={institution_data.get('FII', {}).get('net_options', {}).get('strength', 'N/A')}, Net OI={institution_data.get('FII', {}).get('net_options', {}).get('net_oi', 0):,.0f}, Change={institution_data.get('FII', {}).get('net_options', {}).get('change', 0):,.0f}

CLIENT (Retail Investors):
- Index Futures: Strength={institution_data.get('CLIENT', {}).get('index_futures', {}).get('strength', 'N/A')}, Net OI={institution_data.get('CLIENT', {}).get('index_futures', {}).get('net_oi', 0):,.0f}, Change={institution_data.get('CLIENT', {}).get('index_futures', {}).get('change', 0):,.0f}
- Call Options: Strength={institution_data.get('CLIENT', {}).get('call_options', {}).get('strength', 'N/A')}, Net OI={institution_data.get('CLIENT', {}).get('call_options', {}).get('net_oi', 0):,.0f}, Change={institution_data.get('CLIENT', {}).get('call_options', {}).get('change', 0):,.0f}
- Put Options: Strength={institution_data.get('CLIENT', {}).get('put_options', {}).get('strength', 'N/A')}, Net OI={institution_data.get('CLIENT', {}).get('put_options', {}).get('net_oi', 0):,.0f}, Change={institution_data.get('CLIENT', {}).get('put_options', {}).get('change', 0):,.0f}
- Net Options: Strength={institution_data.get('CLIENT', {}).get('net_options', {}).get('strength', 'N/A')}, Net OI={institution_data.get('CLIENT', {}).get('net_options', {}).get('net_oi', 0):,.0f}, Change={institution_data.get('CLIENT', {}).get('net_options', {}).get('change', 0):,.0f}

PRO (Proprietary Traders):
- Index Futures: Strength={institution_data.get('PRO', {}).get('index_futures', {}).get('strength', 'N/A')}, Net OI={institution_data.get('PRO', {}).get('index_futures', {}).get('net_oi', 0):,.0f}, Change={institution_data.get('PRO', {}).get('index_futures', {}).get('change', 0):,.0f}
- Call Options: Strength={institution_data.get('PRO', {}).get('call_options', {}).get('strength', 'N/A')}, Net OI={institution_data.get('PRO', {}).get('call_options', {}).get('net_oi', 0):,.0f}, Change={institution_data.get('PRO', {}).get('call_options', {}).get('change', 0):,.0f}
- Put Options: Strength={institution_data.get('PRO', {}).get('put_options', {}).get('strength', 'N/A')}, Net OI={institution_data.get('PRO', {}).get('put_options', {}).get('net_oi', 0):,.0f}, Change={institution_data.get('PRO', {}).get('put_options', {}).get('change', 0):,.0f}
- Net Options: Strength={institution_data.get('PRO', {}).get('net_options', {}).get('strength', 'N/A')}, Net OI={institution_data.get('PRO', {}).get('net_options', {}).get('net_oi', 0):,.0f}, Change={institution_data.get('PRO', {}).get('net_options', {}).get('change', 0):,.0f}

Please provide a quick summary covering: Overall market sentiment combining futures and options positioning and expected market direction with confidence level

Keep the summary professional, clear and actionable for traders."""

    # Call OpenRouter API with Grok 4 fast
    try:
        response = client.chat.completions.create(
            model="x-ai/grok-4-fast",  # Grok 4 fast model
            messages=[
...
if __name__ == "__main__":
    ...
    api_key = "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"  # Get from https://openrouter.ai/keys
    
    summary = generate_market_summary(results, pd.to_datetime('2025-10-07'), api_key)
    print(summary)